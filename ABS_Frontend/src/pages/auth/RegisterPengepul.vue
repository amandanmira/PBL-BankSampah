<template>
  ...
  <div class="fixed top-0 left-0 w-full h-[60px] bg-[#4A7043] flex items-center justify-between px-12 z-50">
    ...
    <div class="hidden sm:flex items-center gap-6">
      ...
      <RouterLink to="/register-nasabah" class="text-white text-[15px] font-bold hover:text-white/80 transition-colors">Buat Akun Nasabah</RouterLink>
    </div>
  </div>

  <div class="min-h-screen w-full bg-[#f6f7f6] font-['Inter'] relative pt-[80px] pb-12">
    <!-- Back Header -->
    <div class="px-8 lg:px-20 mb-[40px]">
    ...
    <main class="flex flex-col items-center max-w-[560px] mx-auto px-4">
      <div class="w-full bg-white rounded-3xl shadow-[0_8px_30px_rgba(0,0,0,0.06)] px-10 py-12 border border-[#E8EAE8]/60">
        ...
        <form @submit.prevent="register" class="w-full flex flex-col gap-5">
          ...
          <div class="flex flex-col gap-2">
            ...
            <input v-model="form.nama" type="text" placeholder="Masukkan nama lengkap" required class="w-full h-[50px] px-4 bg-[#eff1ef] rounded-xl text-[15px] outline-none focus:ring-2 focus:ring-[#4A7043]/30 transition-all text-[#5E6460] placeholder-[#a9b0aa]" />
          </div>

          <div class="flex flex-col gap-2">
            <label class="text-[#73A36B] text-[15px] font-bold tracking-wide">Nama Lembaga</label>
            <input v-model="form.nama_lembaga" type="text" placeholder="Masukkan nama lembaga" required class="w-full h-[50px] px-4 bg-[#eff1ef] rounded-xl text-[15px] outline-none focus:ring-2 focus:ring-[#4A7043]/30 transition-all text-[#5E6460] placeholder-[#a9b0aa]" />
          </div>

          <div class="flex flex-col gap-2">
            <label class="text-[#73A36B] text-[15px] font-bold tracking-wide">Alamat</label>
            <input v-model="form.alamat" type="text" placeholder="Masukkan alamat" required class="w-full h-[50px] px-4 bg-[#eff1ef] rounded-xl text-[15px] outline-none focus:ring-2 focus:ring-[#4A7043]/30 transition-all text-[#5E6460] placeholder-[#a9b0aa]" />
          </div>

          <div class="flex flex-col gap-2">
            <label class="text-[#73A36B] text-[15px] font-bold tracking-wide">Nomor Telepon</label>
            <input v-model="form.no_telp" type="tel" placeholder="Masukkan nomor telepon" required class="w-full h-[50px] px-4 bg-[#eff1ef] rounded-xl text-[15px] outline-none focus:ring-2 focus:ring-[#4A7043]/30 transition-all text-[#5E6460] placeholder-[#a9b0aa]" />
          </div>
