<template>
  <div class="container">
    <h1>Riwayat Penarikan (Manager)</h1>
...
const fetchData = async () => {
  try {
    ...
    const response = await axios.get("http://localhost:8000/api/manager/riwayat-penarikan", { headers });
...
const openDetail = async (id) => {
  ...
  try {
    const token = sessionStorage.getItem("token");
    const headers = { Authorization: `Bearer ${token}` };
    const response = await axios.get(`http://localhost:8000/api/manager/riwayat-penarikan/${id}`, { headers });
