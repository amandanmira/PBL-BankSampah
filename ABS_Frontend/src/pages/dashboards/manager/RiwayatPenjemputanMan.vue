<template>
  <div class="container">
    <h1>Riwayat Penjemputan (Manager)</h1>
    ...
    <table v-else-if="allPenjemputans.length > 0">
      ...
      <tbody>
        <tr v-for="p in allPenjemputans" :key="p.penjemputan_id">
          ...
          <td>
            <button v-if="p.status !== 'pending' && p.status !== 'proses'" @click="openDetail(p.penjemputan_id)" class="btn-show">
              Lihat Detail
            </button>
          </td>
        </tr>
...
const fetchData = async () => {
  try {
    ...
    const response = await axios.get("http://localhost:8000/api/manager/riwayat-penjemputan", { headers });
...
const openDetail = async (id) => {
  ...
  try {
    const token = sessionStorage.getItem("token");
    const headers = { Authorization: `Bearer ${token}` };
    const response = await axios.get(`http://localhost:8000/api/manager/riwayat-penjemputan/${id}`, { headers });
