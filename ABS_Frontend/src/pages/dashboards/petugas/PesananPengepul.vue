...
const currentGudangNama = computed(() => user.value.gudang?.alamat || 'Alamat tidak tersedia')
...
const getTugasAndaItems = (detailTransaksi) => {
  ...
  return detailTransaksi.filter(
    (d) => Number(d.sampah?.gudang_id) === Number(currentGudangId.value),
  )
}
...
const getPendingOtherWarehouses = (order) => {
  ...
  order.detail_transaksi.forEach((d) => {
    ...
    if (gid !== currentGudangId.value && d.status === 'pending') {
      const gName = d.sampah?.gudang?.alamat || 'Alamat tidak tersedia'
...
const isCollectionCompletedByCurrentWarehouse = (order) => {
  ...
  return tugasAnda.every((item) => {
    ...
    return (
      ['selesai', 'diambil', 'diserahkan', 'success'].includes(s) ||
...
const confirmApproveStock = async () => {
  ...
  try {
    await axios.put(
      `/api/petugas/pesanan-pengepul/${selectedOrder.value.transaksi_id}/approve-stok`,
    )
    ...
    Swal.fire(
      'Gagal',
      err.response?.data?.message || 'Terjadi kesalahan saat menyetujui stok.',
      'error',
    )
  } finally {
...
const confirmRejectOrder = async () => {
  ...
  try {
    ...
    Swal.fire(
      'Gagal',
      err.response?.data?.message || 'Terjadi kesalahan saat menolak pesanan.',
      'error',
    )
  } finally {
    isSubmitting.value = false
...
const confirmApprovePayment = async () => {
  ...
  try {
    await axios.put(
      `/api/petugas/pesanan-pengepul/${selectedOrder.value.transaksi_id}/verifikasi-pembayaran`,
    )
...
const confirmRejectPayment = async () => {
  ...
  try {
    ...
    Swal.fire(
      'Gagal',
      err.response?.data?.message || 'Terjadi kesalahan saat menolak bukti.',
      'error',
    )
  } finally {
    isSubmitting.value = false
  ...
  <DashboardLayout title="Pesanan Pengepul">
    <div class="space-y-6 animate-in fade-in duration-700 pb-10">
      <div class="flex justify-between items-start">
        <div>
          ...
          <p class="text-stone-500 font-medium">
            Validasi stok, pembayaran, dan pantau pengambilan sampah oleh Pengepul.
          </p>
        </div>
      </div>
      ...
      <div class="bg-white rounded-2xl p-4 shadow-sm border border-stone-100 relative">
        <div class="relative group">
          <Icon
            ...
            class="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-stone-300 group-focus-within:text-[#4A7043] transition-colors"
          />
          <input
            v-model="searchQuery"
      ...
      <div class="space-y-4">
        <div v-if="loading && orders.length === 0" class="space-y-4">
          <div
            v-for="i in 3"
            :key="i"
            class="bg-white rounded-3xl p-6 border border-stone-100 animate-pulse h-60"
          ></div>
        </div>

        <div
          v-else-if="orders.length === 0"
          class="bg-white rounded-3xl p-20 shadow-sm border border-stone-100 text-center"
        >
          <div class="flex flex-col items-center opacity-40">
            ...
            <p class="text-sm text-stone-500 mt-1">
              Tidak ada pesanan pengepul yang terdaftar pada filter ini.
            </p>
          </div>
        </div>
        ...
        >
            ...
            <div class="flex flex-col gap-1.5">
              <div class="flex items-center gap-2">
                <span class="text-base font-bold text-stone-800"
                  >PO-{{ String(order.transaksi_id).padStart(3, '0') }}</span
                >
              ...
              <span
                class="text-xs font-bold text-[#4A7043] bg-[#E8F0E6] px-2.5 py-1 rounded-md w-max"
                >{{ order.pengepul?.nama_lembaga || 'Unknown' }}</span
              >
            </div>
            <span
              :class="[
          ...
          <div class="space-y-4">
            <div class="bg-[#F5F8F5] border border-[#4A7043]/20 rounded-2xl p-6 space-y-4">
              ...
              <div class="space-y-3">
                <div
                  v-if="getTugasAndaItems(order.detail_transaksi).length === 0"
                  class="text-[#4A7043]/60 text-xs font-semibold py-1"
                >
            ...
            <div
              v-if="getGudangLainItems(order.detail_transaksi).length > 0"
              class="bg-white border border-stone-200 rounded-2xl p-6 space-y-4"
            >
              <h4 class="text-sm font-bold text-stone-400">Item Gudang Lain</h4>
              <div class="space-y-3">
                <div
          ...
          <div
            class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 pt-4 border-t border-stone-100"
          >
            <div class="text-sm text-stone-500 font-medium">
              ...
              <span class="text-base font-bold text-stone-800">{{
                formatRupiah(
                  order.total_harga ||
                    order.detail_transaksi.reduce((a, d) => a + d.berat * d.harga, 0),
                )
              }}</span>
            </div>

            <div
              class="w-full md:w-auto flex flex-wrap items-center justify-end gap-3 self-stretch md:self-auto"
            >
              <button
              ...
              <template v-if="activeFilter === 'perlu_validasi'">
                <template v-if="order.status === 'pending'">
                  <div
                    v-if="isCurrentWarehouseApproved(order)"
                    class="flex items-center gap-2 text-stone-500 text-xs font-semibold py-2"
                  >
                    <Icon icon="line-md:loading-twotone-loop" class="w-4 h-4 text-orange-500" />
                    <span
                      >Menunggu konfirmasi
                      {{ getPendingOtherWarehouses(order).join(', ') }}...</span
                    >
                  </div>
                ...
                <template v-else-if="order.status === 'proses' && order.bukti_transfer">
                  <div
                    v-if="isPaymentVerifiedByCurrentWarehouse(order)"
                    class="flex items-center gap-2 text-stone-500 text-xs font-semibold py-2"
                  >
                    <Icon icon="line-md:loading-twotone-loop" class="w-4 h-4 text-orange-500" />
                    <span
                      >Pembayaran terverifikasi ({{ currentGudangNama }}). Menunggu gudang
                      lain...</span
                    >
                  </div>
              ...
              <template v-else-if="activeFilter === 'siap_diambil'">
                <div
                  v-if="isCollectionCompletedByCurrentWarehouse(order)"
                  class="flex items-center gap-2 text-stone-500 text-xs font-semibold py-2"
                >
                  <Icon icon="line-md:loading-twotone-loop" class="w-4 h-4 text-orange-500" />
                  <span
                    >Barang telah diserahkan ({{ currentGudangNama }}). Menunggu gudang
                    lain...</span
                  >
                </div>
      ...
      <div
        ...
        class="bg-white rounded-2xl shadow-sm border border-stone-100 p-4 flex flex-col md:flex-row justify-between items-center gap-4"
      >
    ...
    <Transition name="fade">
      <div v-if="selectedOrder" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <div
          v-if="isConfirmApproveOpen"
          class="relative bg-white w-full max-w-md rounded-[2rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 p-6 space-y-6"
        >
          <div class="flex justify-between items-start">
            <h3 class="text-lg font-black text-stone-800">Konfirmasi Persetujuan</h3>
            <button
              @click="closeAllModals"
              class="p-2 hover:bg-stone-50 rounded-xl transition-colors text-stone-400"
            >
          ...
          <div class="space-y-4">
            <div
              class="w-16 h-16 rounded-full bg-green-50 text-[#4A7043] flex items-center justify-center mx-auto shadow-sm"
            >
            ...
            <p class="text-sm font-medium text-stone-600 text-center leading-relaxed">
              Apakah Anda yakin stok untuk pesanan
              <span class="font-extrabold text-stone-800"
                >PSN-{{ String(selectedOrder.transaksi_id).padStart(3, '0') }}</span
              >
              di <span class="font-extrabold text-stone-800">{{ currentGudangNama }}</span> sudah
              tersedia dan siap disisihkan?
            </p>
          </div>
        ...
        <div
          v-if="isConfirmRejectOpen"
          class="relative bg-white w-full max-w-md rounded-[2rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 p-6 space-y-6"
        >
          <div class="flex justify-between items-start">
            <h3 class="text-lg font-black text-stone-800">
              Tolak Pesanan PSN-{{ String(selectedOrder.transaksi_id).padStart(3, '0') }}
            </h3>
            <button
              @click="closeAllModals"
              class="p-2 hover:bg-stone-50 rounded-xl transition-colors text-stone-400"
        ...
        <div
          v-if="isProofModalOpen"
          class="relative bg-white w-full max-w-lg rounded-[2rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 p-6 space-y-6"
        >
          <div class="flex justify-between items-start border-b border-stone-50 pb-4">
            <div>
              ...
              <p class="text-xs text-stone-400 font-bold">
                PSN-{{ String(selectedOrder.transaksi_id).padStart(3, '0') }} • 
                {{ selectedOrder.pengepul?.nama_lembaga || '-' }}
              </p>
            </div>
            <button
              @click="closeAllModals"
          ...
          <div
            class="flex justify-center bg-stone-50 rounded-2xl p-4 overflow-hidden border border-stone-100"
          >
            <img
        ...
        <div
          v-if="isApprovePaymentModalOpen"
          class="relative bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 p-6 space-y-6"
        >
          <div class="space-y-3">
            ...
            <p class="text-sm font-medium text-slate-600 leading-relaxed">
              Apakah Anda yakin dana sebesar
              <span class="font-bold text-slate-900">{{
                formatRupiah(
                  selectedOrder.total_harga ||
                    selectedOrder.detail_transaksi.reduce((a, d) => a + d.berat * d.harga, 0),
                )
              }}</span>
          ...
          <div class="space-y-4">
            ...
            <div class="space-y-2">
              <label class="text-sm font-semibold text-slate-700"
                >Alasan Bukti Tidak Valid <span class="text-red-500">*</span></label
              >
        ...
        <div
          ...
          class="relative bg-white w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 p-0 flex flex-col max-h-[90vh]"
        >
          <div class="flex justify-between items-center p-6 border-b border-stone-100">
            <h3 class="text-lg font-black text-stone-800">
              Detail Pesanan PSN-{{ String(selectedOrder.transaksi_id).padStart(3, '0') }}
            </h3>
            <button
              @click="closeAllModals"
              class="p-2 hover:bg-stone-50 rounded-xl transition-colors text-stone-400"
          ...
          <div class="p-6 overflow-y-auto no-scrollbar space-y-6">
            <div class="bg-stone-50 rounded-2xl p-4 border border-stone-100 space-y-3">
              <h4 class="text-xs font-bold text-stone-400 uppercase tracking-wider">
                Informasi Pengepul
              </h4>
            ...
            <div
              v-if="['tolak', 'batal'].includes(selectedOrder.status)"
              class="bg-red-50 rounded-2xl p-4 border border-red-100 space-y-2"
            >
              <h4 class="text-xs font-bold text-red-500 uppercase tracking-wider">
                Alasan Pembatalan / Penolakan
              </h4>
              <p class="text-sm font-semibold text-red-700 leading-relaxed">
                {{ selectedOrder.ket_status || 'Tidak ada keterangan yang dicantumkan.' }}
              </p>
            </div>
            ...
            <div class="space-y-3">
              ...
              <div class="bg-white border border-stone-100 rounded-2xl overflow-hidden shadow-sm">
                <div
                  v-for="(item, index) in selectedOrder.detail_transaksi"
                  :key="item.detail_id"
                  class="p-4 flex justify-between items-center"
                  :class="{ 'border-t border-stone-50': index > 0 }"
                >
                  <div class="flex-1">
                    <p class="text-sm font-bold text-stone-800">
                      {{ item.sampah?.item_sampah?.nama }}
                    </p>
                    <p class="text-xs text-stone-500 mt-0.5">
                      {{ item.sampah?.gudang?.alamat || 'Alamat tidak tersedia' }}
                    </p>
                  </div>
                  <div class="text-right">
                    <p class="text-sm font-bold text-stone-800">
                      {{ item.berat }} kg
                      <span class="text-xs text-stone-400 font-normal ml-1"
                        >× {{ formatRupiah(item.harga) }}</span
                      >
                    </p>
                    <p class="text-sm font-bold text-[#4A7043] mt-0.5">
                      {{ formatRupiah(item.berat * item.harga) }}
                    </p>
                  </div>
                </div>
                <div
                  class="bg-stone-50 p-4 border-t border-stone-100 flex justify-between items-center"
                >
            ...
            <div class="grid grid-cols-2 gap-4">
              ...
              <div
                ...
                class="bg-green-50 rounded-2xl p-4 border border-green-100"
              >
        ...
        <div
          v-if="isCompleteCollectionModalOpen"
          class="relative bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 p-6 space-y-6"
        >
          <div class="space-y-3">
            <h3 class="text-lg font-bold text-slate-900">Konfirmasi Persetujuan</h3>
